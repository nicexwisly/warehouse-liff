...
export default function SearchPage({ profile }) {
  ...
  // สแกน barcode แล้วค้นหาเลย
  async function handleScanResult(val) {
    setShowScanner(false)
    setQ(val)
    // ถ้า barcode 13 หลัก → ลองค้นจาก master ก่อนเพื่อได้ item_code จริง
    let searchVal = val
    if (/^\d{8,}$/.test(val)) {
      try {
      ...
      <div style={styles.page}>
        <div style={styles.header}>
          ...
          <div style={styles.searchRow}>
            <input
              style={styles.input}
              placeholder="ชื่อ, รหัสสินค้า หรือ barcode..."
              value={q}
              onChange={e => setQ(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && handleSearch()}
              autoFocus
            />
            <button onClick={() => setShowScanner(true)} style={styles.scanBtn} title="สแกน barcode">
              📷
            </button>
        ...
        <div style={styles.body}>
          {searched && !loading && (
            <p style={styles.count}>
              {results.length > 0 ? `พบ ${results.length} รายการ` : 'ไม่พบสินค้านี้ในระบบ'}
            </p>
          )}
          {results.map(item => (
...
const styles = {
  ...
  scanBtn: { padding: '11px 13px', background: '#f0f0f0', border: 'none', borderRadius: 10, fontSize: 18, cursor: 'pointer' },
  searchBtn: { padding: '11px 16px', background: '#06c755', color: '#fff', border: 'none', borderRadius: 10, fontWeight: 600, fontSize: 14, cursor: 'pointer' },
